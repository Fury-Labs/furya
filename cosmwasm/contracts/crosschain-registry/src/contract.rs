...
mod test {
    ...
    fn query_chain_and_channel() {
        ...
        // Check that furya' channel-42 is connected to juno
        ...
        // Check that furya' channel-75 is connected to stars
        let destination_chain = query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::GetDestinationChainFromSourceChainViaChannel {
        ...
        // Retrieve stargaze<>furya link and check the channel is what we expect
        let channel_binary = query(
            deps.as_ref(),
            mock_env(),
            QueryMsg::GetChannelFromChainPair {
