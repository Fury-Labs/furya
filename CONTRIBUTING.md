...
The following information provides a set of guidelines for contributing to the Furya chain dev repo. Use your best judgment, and, if you see room for improvement, please propose changes to this document.

The contributing guide for Furya explains the branching structure, how to use the SDK fork, how to make / test updates to SDK branches and how to create release notes.

Contributions come in the form of writing documentation, raising issues / PRs, and any other actions that help develop the Furya protocol documentation.

## First steps

The first step is to find an issue you want to fix. To identify issues we think are good for first-time contributors, we add the **good first issue** label. [You can see a list of issues to contribute here](https://github.com/furya-labs/furya/contribute).

We recommend setting up your IDE as per our [recommended IDE setup](https://docs.furya.zone/furya-core/ide-guide) before proceeding.

If you have a feature request, please use the [feature-request repo](https://github.com/furya-labs/feature-requests). We also welcome you to [make an issue](https://github.com/furya-labs/furya/issues/new/choose) for anything of substance, or posting an issue if you want to work on it.
...
1. [Fork the repository](https://github.com/furya-labs/furya).
...
7. Next, rinse and repeat the following:
    ...
    3. Create a PR on the Furya repository. There should be a PR template to help you do so.
...
We'll lay out three examples below (one that uses our format for messages, one that applies to keeper methods, and one that applies to our GAMM module), each of which will hopefully be simple enough to copy-paste into a test file and use as a starting point for your test-writing in the Furya Core repo.
...
Note: this section assumes you already have the Go plugin for Vscode installed. Please refer to our [IDE setup docs](https:/docs.furya.zone/furya-core/ide-guide) if you haven't done any IDE setup yet.

Copy the `templates` folder into your `.vscode` folder from our main repo [here](https://github.com/furya-labs/furya/tree/main/.vscode). This folder has our custom templates for generating tests that fit our testing standards as accurately as possible.
...
where `"[ABSOLUTE PATH TO TEMPLATES FOLDER]"` should look something like: `"User/ExampleUser/furya/.vscode/templates"`
...
Since the GAMM module is core to the Furya repo, it might be useful to have a good example of a well-structured GAMM-specific test. This example covers a simple getter function and validates the specific error messages around the function (as opposed to merely the presence of an error):
...
The e2e package defines an integration testing suite used for full end-to-end testing functionality. This package is decoupled from depending on the Furya codebase. It initializes the chains for testing via Docker files. 

As a result, the test suite may provide the desired Furya version to Docker containers during the initialization. This design allows for the opportunity of testing chain upgrades in the future by providing an older Furya version to the container, performing the chain upgrade, and running the latest test suite. 
...
Additionally, there is an ability to disable certain components of the e2e suite. This can be done by setting the environment variables. See the [E2E test docs](https://github.com/furya-labs/furya/blob/main/tests/e2e/README.md)  or more details.
...
```sh
go get github.com/furya-labs/cosmos-sdk@{my commit hash}
```

You get something like:

```sh
go get: github.com/furya-labs/cosmos-sdk@v0.33.2 updating to
github.com/furya-labs/cosmos-sdk@v0.42.10-0.20210829064313-2c87644925da: parsing go.mod:
module declares its path as: github.com/cosmos/cosmos-sdk
but was required as: github.com/furya-labs/cosmos-sdk
```
...
You can do rm -rf vendor and make build-reproducible to redownload all dependencies - this should pull the latest docker image of Furya. You should also make sure to do make proto-all to auto-generate your protobuf files. Makes ure you have docker installed.
...
- The GitHub release is created in our private repo via this [GitHub workflow](https://github.com/furya-labs/furya-ci/blob/main/.github/workflows/create-release.yaml). The workflow is manually triggered from the [furya-ci repository](https://github.com/furya-labs/furya-ci). The workflow uses the `make build-reproducible` command to create the `furyad` binaries using the default [Makefile](https://github.com/furya-labs/furya/blob/main/Makefile#L99).
...
- (Eventually) Make a PR that adds a version handler for the next upgrade
  - [Add v10 upgrade boilerplate #1649](https://github.com/furya-labs/furya/pull/1649/files)
...
following:
<https://github.com/furya-labs/furya/blob/8ef2f1845d9c7dd3f422d3f1953e36e5cf112e73/x/mint/types/errors.go#L8-L10>
...
We communicate with various integrators if they'd like release-blocking QA testing for major releases
    * Chainapsis has communicated wanting a series of furya-frontend functionalities to be checked for correctness on a testnet as a release blocking item
...
[4]:https://github.com/furya-labs/cosmos-sdk/blob/5c9a51c277d067e0ec5cf48df30a85fae95bcd14/store/rootmulti/store.go#L430
[5]:https://github.com/furya-labs/cosmos-sdk/blob/5c9a51c277d067e0ec5cf48df30a85fae95bcd14/store/types/commit_info.go#L40

## Common Security Considerations

There are several security patterns that come up frequently enough to be synthesized into general rules of thumb. While the high level risks are appchain agnostic, the details are mostly tailored to contributing to Furya. This is, of course, not even close to a complete list – just a few considerations to keep in mind.
...
## Debug Furya Node VS Code & Delve
...
2. Start Furya node with the binary from step 1.

3. Run "Attach to running furyad process" VS Code debug configuration

What it does:
- Runs a vs code background task that starts a delve server and attaches to the Furya node process ID
- Attaches VS code project to delve and allows you to set breakpoints

FAQ
- Can this be used with localfurya or inside Docker?
