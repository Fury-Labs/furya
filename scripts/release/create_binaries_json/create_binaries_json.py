...
for a given release tag of fury-labs/furya or from a provided checksum URL.
...
❯ python create_binaries_json.py --checksums_url https://github.com/fury-labs/furya/releases/download/v16.1.1/sha256sum.txt

Output:
{
    "binaries": {
    "linux/arm64": "https://github.com/fury-labs/furya/releases/download/16.1.1/furyad-16.1.1-linux-arm64?checksum=<checksum>",
    "darwin/arm64": "https://github.com/fury-labs/furya/releases/download/16.1.1/furyad-16.1.1-darwin-arm64?checksum=<checksum>",
    "darwin/amd64": "https://github.com/fury-labs/furya/releases/download/16.1.1/furyad-16.1.1-darwin-amd64?checksum=<checksum>,
    "linux/amd64": "https://github.com/fury-labs/furya/releases/download/16.1.1/furyad-16.1.1-linux-amd64?checksum=><checksum>"
    }
}

Expects a checksum in the form:

<CHECKSUM>  furyad-<VERSION>-<OS>-<ARCH>[.tar.gz]
<CHECKSUM>  furyad-<VERSION>-<OS>-<ARCH>[.tar.gz]
...

Example:

f838618633c1d42f593dc33d26b25842f5900961e987fc08570bb81a062e311d  furyad-16.1.1-linux-amd64
fa6699a763487fe6699c8720a2a9be4e26a4f45aafaec87aa0c3aced4cbdd155  furyad-16.1.1-linux-amd64.tar.gz

(From: https://github.com/fury-labs/furya/releases/download/v16.1.1/sha256sum.txt)
...
https://github.com/fury-labs/furya/releases/download/<TAG>/sha256sum.txt
"""
...
def checksums_to_binaries_json(checksums):
    ...
    for line in checksums.splitlines():
        ...
        if not filename.endswith('.tar.gz') and filename.startswith('furyad'):
            try:
                _, tag, platform, arch = filename.split('-')
            except ValueError:
                print(f"Error: Expected binary name in the form: furyad-X.Y.Z-platform-architecture, but got {filename}")
                sys.exit(1)
            _, tag, platform, arch,  = filename.split('-')
...
def main():
    ...
    checksums_url = args.checksums_url if args.checksums_url else f"https://github.com/fury-labs/furya/releases/download/{args.tag}/sha256sum.txt"
