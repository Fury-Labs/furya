...
# https://quicksync.io/networks/furya.html
# 2nd NB: you can change FURYAD_PRUNING=nothing to FURYAD_PRUNING=default OR you could also set the pruning settings manually with FURYAD_PRUNING=custom
...
export FURYAD_PRUNING=nothing
export FURYAD_DB_BACKEND=pebbledb
export FURYAD_P2P_MAX_NUM_OUTBOUND_PEERS=500
export FURYAD_P2P_MAX_NUM_INBOUND_PEERS=500
export FURYAD_P2P_SEEDS=$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/furya/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')
export FURYAD_P2P_LADDR=tcp://0.0.0.0:2001
...
go mod edit -replace github.com/tendermint/tm-db=github.com/notional-labs/tm-db@136c7b6
go mod tidy
go install -ldflags '-w -s -X github.com/cosmos/cosmos-sdk/types.DBBackend=pebbledb' -tags pebbledb ./...time furyad start --db_backend pebbledb
furyad start --db_backend pebbledb
git reset --hard
