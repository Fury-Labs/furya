...
# 3) update FURYA_E2E_UPGRADE_VERSION variable in .vscode/launch.json
...
# Github workflow which calls this script can be found here: furya/.github/workflows/auto-update-upgrade.yml
...
# constants.go logic
echo "// UpgradeName defines the on-chain upgrade name for the Furya $version_create upgrade." >> $CONSTANTS_FILE
...
if [[ $version_create == v$(($(echo $PREV_FURYA_E2E_TAG | awk -F . '{print $1}' | grep -Eo '[0-9]*')+1)) ]]; then	
    echo "Found previous furya-e2e-init-chain tag $PREV_FURYA_E2E_TAG"
	sed -i '/previousVersionInitTag/s/".*"/'"\"$PREV_FURYA_E2E_TAG\""'/' $e2e_file
else
    PREV_FURYA_E2E_TAG=v$((${version_create:1}-1)).0.0
    echo "Using pre-defined furya-e2e-init-chain tag: $PREV_FURYA_E2E_TAG"
    sed -i '/previousVersionInitTag/s/".*"/'"\"$PREV_FURYA_E2E_TAG\""'/' $e2e_file
fi

# update FURYA_E2E_UPGRADE_VERSION in launch.json
sed -i "s/${bracks}FURYA_E2E_UPGRADE_VERSION${bracks}: ${bracks}v$latest_version${bracks}/${bracks}FURYA_E2E_UPGRADE_VERSION${bracks}: ${bracks}$version_create${bracks}/" ./.vscode/launch.json
