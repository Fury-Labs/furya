...
done < <(find . -name go.mod -exec grep -l "github.com/fury-labs/furya/v20" {} \; | grep -v  "^./go.mod$" | sed 's|/go.mod||' | sed 's|^./||')

replace_paths() {
    file="${1}"
    sed -i "s/github.com\/fury-labs\/furya\/v${version_to_replace}/github.com\/fury-labs\/furya\/v${NEXT_MAJOR_VERSION}/g" ${file}
}
