...
furyad add-genesis-account $(furyad keys show validator -a --keyring-backend=test --home=$HOME/.furyad) 1000000000uosmo,1000000000valtoken --home=$HOME/.furyad
sed -i -e "s/stake/uosmo/g" $HOME/.furyad/config/genesis.json
furyad gentx validator 500000000uosmo --commission-rate="0.0" --keyring-backend=test --home=$HOME/.furyad --chain-id=testing
...
furyad tx gov submit-proposal --title="prop with enough osmo deposit" --description="prop w/ enough deposit"  --from=validator --deposit=500000000uosmo --chain-id=testing --keyring-backend=test --broadcast-mode=block  --type="Text"
