...
update_genesis () {    
    cat $HOME/.furyad/config/genesis.json | jq "$1" > $HOME/.furyad/config/tmp_genesis.json && mv $HOME/.furyad/config/tmp_genesis.json $HOME/.furyad/config/genesis.json
}
...
tmux new -s validator -d furyad start --home=$HOME/.furyad
...
furyad tx lockup lock-tokens 10000000000000000000gamm/pool/1 --duration=360h --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# register superfluid asset through governance
furyad tx gov submit-proposal set-superfluid-assets-proposal --title="set superfluid assets" --description="set superfluid assets description" --superfluid-assets="gamm/pool/1" --deposit=10000000stake --from=validator --chain-id=testing --keyring-backend=test --broadcast-mode=block --yes --fees=10000stake
sleep 7
furyad tx gov deposit 1 10000000stake --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake 
sleep 7
furyad tx gov vote 1 yes --from=validator --keyring-backend=test --chain-id=testing --yes --fees=100000stake
sleep 7

# Command to extract operator_address
operator_address=$(./furyad q staking validators | yq e '.validators[].operator_address' -)

# Use the operator_address in the furyad command
furyad tx superfluid delegate 1 ${operator_address} --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# add different account that would be set as lock reward receiver
furyad keys add test1 --keyring-backend=test 
sleep 7

# set lock reward receiver 
furyad tx lockup set-reward-receiver-address 1 $(./furyad keys show test1 -a --keyring-backend=test) --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# query lock reward receiver 
# furyad q lockup lock-reward-receiver 1

# the new address would be receiving the lock rewards, while the original account does not receive the rewards anymore
# furyad q bank balances $(furyad keys show test1 -a --keyring-backend=test)
# furyad q bank balances $(furyad keys show validator -a --keyring-backend=test)

# set the lock reward receiver back to the original address
furyad tx lockup set-reward-receiver-address 1 $(./furyad keys show validator -a --keyring-backend=test) --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7
...
furyad tx lockup lock-tokens 10000000000000000000gamm/pool/1 --duration=1s --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7

# now let's also try adding lock incentives as well
furyad tx incentives create-gauge gamm/pool/1 1000000uion 0 --duration 120s  --start-time 1688115600 --epochs 100 --from=validator --keyring-backend=test --chain-id=testing --broadcast-mode=block --yes --fees=10000stake
sleep 7
