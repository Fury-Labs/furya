...
furyad tx gov submit-proposal wasm-store $CONTRACT.wasm --title "Add $CONTRACT" \
...
furyad query gov proposal $PROPOSAL

furyad tx gov deposit $PROPOSAL 10000000stake --from validator --keyring-backend test \
    --chain-id $CHAIN_ID -y -b block --gas 5000000 --gas-prices 0.025stake

furyad tx gov vote $PROPOSAL yes --from validator --keyring-backend test \
