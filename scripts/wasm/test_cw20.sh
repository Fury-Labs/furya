...
furyad keys add demo --keyring-backend test
VAL=$(furyad keys show -a validator --keyring-backend test)
DEMO=$(furyad keys show -a demo --keyring-backend test)
...
furyad tx wasm instantiate $CODE "$INIT" --label "First Coin" --no-admin --from validator \
...
furyad query wasm contract-state smart $CONTRACT "$QUERY"
echo "Demo Balance:"
furyad query wasm contract-state smart $CONTRACT "$QUERY_DEMO"
...
furyad tx wasm execute $CONTRACT "$TRANSFER" --from validator --keyring-backend test \
