...
use furya_std_derive::CosmwasmExt;
...
// furya-std doesn't currently support the SupplyOf query, so I'm defining it localy so it can be used to obtain the channel value
#[derive(
    Clone,
    PartialEq,
    Eq,
...
pub struct QuerySupplyOfResponse {
    ...
    pub amount: ::core::option::Option<furya_std::types::cosmos::base::v1beta1::Coin>,
}
...
pub mod tests {
    ...
    fn sanity_check() {
        ...
        assert_eq!(
            packet.local_denom(&FlowType::In),
            WRAPPED_ATOM_ON_FURYA_HASH.clone()
        );

        // uatom on furya sent back to the hub
        let packet = Packet::mock(
            format!("channel-0"),                      // from: furya
            format!("channel-141"),                    // to: hub
            WRAPPED_ATOM_ON_FURYA_TRACE.to_string(), // unwrapped before reaching the contract
            0_u128.into(),
        );
        ...
        // fury on the hub sent back to furya
        // send
        let packet = Packet::mock(
            format!("channel-141"),                // from: hub
            format!("channel-0"),                  // to: furya
        ...
        // utaom on the furya sent back to the hub
        // send
        let packet = Packet::mock(
            format!("channel-0"),                      // from: furya
            format!("channel-141"),                    // to: hub
    ...
    fn sanity_double() {
        ...
        let juno_wrapped_furya_wrapped_atom_hash =
            "ibc/6CDD4663F2F09CD62285E2D45891FC149A3568E316CE3EBBE201A71A78A69388";

        // Send uatom on stored on furya to juno
        // send
        let packet = Packet::mock(
            format!("channel-42"),                     // from: furya
        ...
        let packet = Packet::mock(
            format!("channel-42"), // from: furya
            format!("channel-0"),  // to: juno
            WRAPPED_ATOM_ON_FURYA_TRACE.to_string(),
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::In),
            juno_wrapped_furya_wrapped_atom_hash
        );

        // Send back that multi-wrapped token to furya
        // send
        let packet = Packet::mock(
            format!("channel-0"),  // from: juno
            format!("channel-42"), // to: furya
            format!("{}{}", "transfer/channel-0/", WRAPPED_ATOM_ON_FURYA_TRACE), // unwrapped before reaching the contract
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::Out),
