...
This is a safety control, intended to protect assets on furya in event of:
...
- [Dragonberry IBC bug](https://forum.cosmos.network/t/ibc-security-advisory-dragonberry/7702) (can't yet disclose amount at risk, but was saved due to being found first by altruistic Furya core developers)

In the presence of a software bug on Furya, IBC itself, or on a counterparty chain, we would like to prevent the bridge from being fully depegged.
...
* Per denomination rate limits
   - allows safety statements like "Only 30% of Stars on Furya can flow out in one day" or "The amount of Atom on Furya can at most double per day".
* Per channel rate limits
   - Limit the total inflow and outflow on a given IBC channel, based on "USDC" equivalent, using Furya as the price oracle.
...
The "Outflow" side of a rate limit is protection against a bug on Furya OR IBC.
...
So this parameterization has to contend with being a tradeoff of withdrawal liveness in high volatility periods vs being a crucial safety rail, in event of on-Furya bug.
...
* Channel: The channel on the Furya side: `packet.SourceChannel` for sends, and `packet.DestinationChannel` for receives. 
* Denom: The denom of the token being transferred as known on the Furya side (more on that bellow)
...
#### Notes on Denom
We always use the the denom as represented on Furya. For native assets that is the local denom, and for non-native 
...
This behaves slightly different if the asset is an furya asset that was sent to the counterparty and is being
...
* For non-native tokens (`ibc/...`), the channel value should be the supply of those tokens in Furya
* For native tokens, the channel value should be the total amount of tokens in escrow across all ibc channels

The later ensures the limits are lower and represent the amount of native tokens that exist outside Furya. This is 
...
Furya for all denoms (i.e.: treat native and non-native tokens the same way). Once that ticket is fixed, we will 
...
This integration can be seen in [furya/app/keepers/keepers.go](https://github.com/furya-labs/furya/blob/main/app/keepers/keepers.go)
...
* Analyze changing denom-based rate limits, to just overall withdrawal amount for Furya