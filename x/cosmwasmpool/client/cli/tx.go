...
func NewCreateCWPoolCmd() (*osmocli.TxCliDesc, *model.MsgCreateCosmWasmPool) {
	return &osmocli.TxCliDesc{
		...
		Example:          "furyad tx cosmwasmpool create-pool 1 '{\"pool_assets_denom\":[\"uion\",\"ufury\"]}' --from lo-test1 --keyring-backend test --chain-id localfurya --fees 875ufury -b=block",
		NumArgs:          2,
...
func NewCmdUploadCodeIdAndWhitelistProposal() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "furyad tx gov submit-proposal upload-code-id-and-whitelist x/cosmwasmpool/bytecode/transmuter.wasm --from lo-test1 --keyring-backend test --title \"Test\" --description \"Test\" -b=block --chain-id localfurya --fees=100000ufury --gas=20000000",
