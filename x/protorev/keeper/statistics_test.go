...
func (s *KeeperTestSuite) TestGetProfitsByDenom() {
	// Should be zero by default
	profits, err := s.App.ProtoRevKeeper.GetProfitsByDenom(s.Ctx, types.FuryaDenomination)
	s.Require().Error(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.ZeroInt()), profits)

	// Pseudo execute a trade
	err = s.App.ProtoRevKeeper.UpdateProfitsByDenom(s.Ctx, types.FuryaDenomination, osmomath.NewInt(9000))
	s.Require().NoError(err)

	// Check the updated result
	profits, err = s.App.ProtoRevKeeper.GetProfitsByDenom(s.Ctx, types.FuryaDenomination)
	s.Require().NoError(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(9000)), profits)

	// Pseudo execute a second trade
	err = s.App.ProtoRevKeeper.UpdateProfitsByDenom(s.Ctx, types.FuryaDenomination, osmomath.NewInt(5000))
	s.Require().NoError(err)

	// Check the updated result after the second trade
	profits, err = s.App.ProtoRevKeeper.GetProfitsByDenom(s.Ctx, types.FuryaDenomination)
	s.Require().NoError(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(14000)), profits)

	// Check the result of GetAllProfits
	allProfits := s.App.ProtoRevKeeper.GetAllProfits(s.Ctx)
	s.Require().Equal([]sdk.Coin{{Denom: types.FuryaDenomination, Amount: osmomath.NewInt(14000)}}, allProfits)
	...
	// Check the result of GetAllProfits
	allProfits = s.App.ProtoRevKeeper.GetAllProfits(s.Ctx)
	s.Require().Equal([]sdk.Coin{{Denom: "Atom", Amount: osmomath.NewInt(1000)}, {Denom: types.FuryaDenomination, Amount: osmomath.NewInt(14000)}}, allProfits)
}
...
func (s *KeeperTestSuite) TestGetProfitsByRoute() {
	...
	profit, err := s.App.ProtoRevKeeper.GetProfitsByRoute(s.Ctx, []uint64{1, 2, 3}, types.FuryaDenomination)
	s.Require().Error(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.ZeroInt()), profit)

	// Pseudo execute a trade
	err = s.App.ProtoRevKeeper.UpdateProfitsByRoute(s.Ctx, []uint64{1, 2, 3}, types.FuryaDenomination, osmomath.NewInt(1000))
	s.Require().NoError(err)

	// Check the updated result
	profit, err = s.App.ProtoRevKeeper.GetProfitsByRoute(s.Ctx, []uint64{1, 2, 3}, types.FuryaDenomination)
	s.Require().NoError(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(1000)), profit)

	// Check the result of GetAllProfitsByRoute
	profits = s.App.ProtoRevKeeper.GetAllProfitsByRoute(s.Ctx, []uint64{1, 2, 3})
	s.Require().Equal([]sdk.Coin{{Denom: types.FuryaDenomination, Amount: osmomath.NewInt(1000)}}, profits)
	...
	// Check the result of GetAllProfitsByRoute
	profits = s.App.ProtoRevKeeper.GetAllProfitsByRoute(s.Ctx, []uint64{1, 2, 3})
	s.Require().Contains(profits, sdk.Coin{Denom: types.FuryaDenomination, Amount: osmomath.NewInt(1000)})
...
func (s *KeeperTestSuite) TestUpdateStatistics() {
	...
	err := s.App.ProtoRevKeeper.UpdateStatistics(s.Ctx,
		...
		types.FuryaDenomination, osmomath.NewInt(1000),
	)
	s.Require().NoError(err)
	...
	err = s.App.ProtoRevKeeper.UpdateStatistics(s.Ctx,
		...
		types.FuryaDenomination, osmomath.NewInt(1100),
	)
	s.Require().NoError(err)
	...
	// Check the result of GetProfitsByRoute
	profit, err = s.App.ProtoRevKeeper.GetProfitsByRoute(s.Ctx, []uint64{2, 3, 4}, types.FuryaDenomination)
	s.Require().NoError(err)
	s.Require().Equal(sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(1100)), profit)
