...
func (s *KeeperTestSuite) TestUpdateHighestLiquidityPools() {
	testCases := []struct {
	...
	}{
		{
			...
			expectedBaseDenomPools: map[string]map[string]keeper.LiquidityPoolStruct{
				"epochOne": {
					"ufury": {Liquidity: osmomath.NewInt(2000000), PoolId: 47},
				},
			},
		},
		{
			...
			expectedBaseDenomPools: map[string]map[string]keeper.LiquidityPoolStruct{
				"epochTwo": {
					"ufury": {Liquidity: osmomath.Int(osmomath.NewUintFromString("999999000000000001000000000000000000")), PoolId: 50},
				},
			},
		},
	}
