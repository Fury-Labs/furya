...
func (suite *KeeperTestSuite) TestSendDeveloperFee() {
	cases := []struct {
		...
		},
		{
			...
			alterState: func() {
				...
				err := suite.pseudoExecuteTrade(types.FuryaDenomination, osmomath.NewInt(1000), 100)
				suite.Require().NoError(err)
			},
			expectedErr:       false,
			expectedDevProfit: sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(20)),
		},
		{
			...
			alterState: func() {
				...
				err := suite.pseudoExecuteTrade(types.FuryaDenomination, osmomath.NewInt(1000), 500)
				suite.Require().NoError(err)
			},
			expectedErr:       false,
			expectedDevProfit: sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(10)),
		},
		{
			...
			alterState: func() {
				...
				err := suite.pseudoExecuteTrade(types.FuryaDenomination, osmomath.NewInt(1000), 1000)
				suite.Require().NoError(err)
			},
			expectedErr:       false,
			expectedDevProfit: sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(5)),
		},
	}
	...
	for _, tc := range cases {
		suite.Run(tc.description, func() {
			...
			err := suite.App.ProtoRevKeeper.SendDeveloperFee(suite.Ctx, sdk.NewCoin(types.FuryaDenomination, osmomath.NewInt(100)))
			if tc.expectedErr {
			...
			if !tc.expectedErr {
				developerFee := suite.App.AppKeepers.BankKeeper.GetBalance(suite.Ctx, developerAccount, types.FuryaDenomination)
