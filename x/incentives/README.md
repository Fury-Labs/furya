...
Anyone can create a gauge and add rewards to the gauge. There is no way to withdraw gauge rewards other than distribution. Governance proposals can be raised to match the external incentive tokens with equivalent Fury incentives (see for example: [proposal 47](https://www.mintscan.io/furya/proposals/47)).
...
```sh
furyad tx incentives create-gauge [lockup_denom] [reward] [flags]
```
...
```bash
furyad tx incentives create-gauge gamm/pool/3 10000ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 \
--duration 24h  --start-time 1640081402 --epochs 2 --from WALLET_NAME --chain-id furya-1
```
...
```bash
furyad tx incentives create-gauge ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 \
1000000000ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED --perpetual --duration 168h \
--from WALLET_NAME --chain-id furya-1
```
...
```sh
furyad tx incentives add-to-gauge [gauge_id] [rewards] [flags]
```
...
```bash
furyad tx incentives add-to-gauge 1914 500000000ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED \
--from WALLET_NAME --chain-id furya-1
```
...
```sh
furyad query incentives active-gauges [flags]
```

::: details Example

```bash
furyad query incentives active-gauges
```
...
```sh
furyad query incentives active-gauges-per-denom [denom] [flags]
```
...
```bash
furyad query incentives active-gauges-per-denom gamm/pool/341
```
...
```sh
furyad query incentives distributed-coins [flags]
```

::: details Example

```bash
furyad query incentives distributed-coins
```
...
```sh
furyad query incentives gauge-by-id [id] [flags]
```
...
```sh
furyad query incentives gauge-by-id 1
```
...
```sh
furyad query incentives gauges [flags]
```
...
```bash
furyad query incentives gauges --limit 2000
```
...
```sh
furyad query incentives to-distribute-coins [flags]
```

::: details Example

```bash
furyad query incentives to-distribute-coins
```
...
```sh
furyad query incentives upcoming-gauges [flags]
```

::: details Example

```bash
furyad query incentives upcoming-gauges
```
