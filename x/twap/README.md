...
In Furya, we maintain accumulator records for every pool, for the last 48 hours.
...
We maintain TWAP accumulation records for every AMM pool on Furya. 

Because Furya supports multi-asset pools, a complicating factor is that we have to store a record for every asset pair in the pool.
...
Each twap record stores [(source)](../../proto/furya/twap/v1beta1/twap_record.proto):
...
- [ ] End to end migration tests
  - Tests that migration of Furya pools created prior to the TWAP upgrade, get TWAPs recorded starting at the v11 upgrade.
- [ ] Integration into the Furya simulator
  - The furya simulator, simulates building up complex state machine states, in random ways not seen before. We plan on, in a property check, maintaining expected TWAPs for short time ranges, and seeing that the keeper query will return the same value as what we get off of the raw price history for short history intervals.
...
- [ ] Mutation testing usage
  - integration of the TWAP module into [go mutation testing](https://github.com/furya-labs/go-mutesting): 
