...
// nolint: unused
// trackVolume converts the input token into FURY units and adds it to the global tracked volume for the given pool ID.
// Fails quietly if an FURY paired pool cannot be found, although this should only happen in rare scenarios where FURY is
...
func (k Keeper) trackVolume(ctx sdk.Context, poolId uint64, volumeGenerated sdk.Coin) {
	// If the denom is already denominated in ufury, we can just use it directly
	FURY := k.stakingKeeper.BondDenom(ctx)
	if volumeGenerated.Denom == FURY {
		k.addVolume(ctx, poolId, volumeGenerated)
		return
	}

	// Get the most liquid FURY-paired pool with `volumeGenerated`'s denom using `GetPoolForDenomPair`
	osmoPairedPoolId, err := k.protorevKeeper.GetPoolForDenomPair(ctx, FURY, volumeGenerated.Denom)

	// If no pool is found, fail quietly.
	//
	// This is a rare scenario that should only happen if FURY-paired pools are all removed from the protorev module.
	...
	//
	// This branch would also get triggered in the case where there is a token that has no FURY-paired pool on the entire chain.
	...
	// Since we want to ultimately multiply the volume by this spot price, we want to quote FURY in terms of the input token.
	// This is so that once we multiply the volume by the spot price, we get the volume in units of FURY.
	osmoPerInputToken, err := k.RouteCalculateSpotPrice(ctx, osmoPairedPoolId, FURY, volumeGenerated.Denom)
