...
func TestLiquidity0(t *testing.T) {
	testCases := map[string]struct {
		...
		"low price range": {
			...
			// from clmath import *
			// from math import *
			// product1 = round_fury_prec_down(sqrtPriceA * sqrtPriceB)
			// product2 = round_fury_prec_down(amount0 * product1)
			// diff = round_fury_prec_down(sqrtPriceB - sqrtPriceA)
...
func TestCalcAmount0Delta(t *testing.T) {
	testCases := map[string]struct {
		...
		// See: https://github.com/fury-labs/furya/issues/6351 for details.
