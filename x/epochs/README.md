...
On Furya mainnet, we only utilize one identifier, with a time interval of `one day`.
...
The Epochs module keeps a single [`EpochInfo`](https://github.com/fury-labs/furya/blob/b4befe4f3eb97ebb477323234b910c4afafab9b7/proto/furya/epochs/genesis.proto#L12) per identifier.
...
```sh
furyad query epochs epoch-infos
```
...
```sh
furyad query epochs current-epoch [identifier]
```
...
```sh
furyad query epochs current-epoch day
```
