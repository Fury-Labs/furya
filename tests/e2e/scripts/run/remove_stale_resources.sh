...
# Filtering by containers belonging to the "furya-testnet" network
LIST_CONTAINERS_CMD=$(docker ps -a --filter network=furya-testnet --format {{.ID}})
LIST_NETWORKS_CMD=$(docker network ls --filter name=furya-testnet --format {{.ID}})
...
for cur_image_sha in $LIST_DOCKER_IMAGE_HASHES; do
    if [[ "$cur_image_sha" != "$local_git_sha" ]]; then
        ...
        docker rmi -f $(docker images --filter=reference="furya:$cur_image_sha" -q)
    fi
done
