...
func (n *NodeConfig) SetMaxPoolPointsPerTx(maxPoolPoints int, from string) {
	...
	cmd := []string{"furyad", "tx", "protorev", "set-max-pool-points-per-tx", fmt.Sprintf("%d", maxPoolPoints), fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
...
func (n *NodeConfig) CreateBalancerPool(poolFile, from string) uint64 {
	...
	cmd := []string{"furyad", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/furya/%s", poolFile), fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
...
func (n *NodeConfig) CreateStableswapPool(poolFile, from string) uint64 {
	...
	cmd := []string{"furyad", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/furya/%s", poolFile), "--pool-type=stableswap", fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CollectSpreadRewards(from, positionIds string) {
	...
	cmd := []string{"furyad", "tx", "concentratedliquidity", "collect-spread-rewards", positionIds, fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateConcentratedPool(from, denom1, denom2 string, tickSpacing uint64, spreadFactor string) uint64 {
	...
	cmd := []string{"furyad", "tx", "concentratedliquidity", "create-pool", denom1, denom2, fmt.Sprintf("%d", tickSpacing), spreadFactor, fmt.Sprintf("--from=%s", from)}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateConcentratedPosition(from, lowerTick, upperTick string, tokens string, token0MinAmt, token1MinAmt int64, poolId uint64) (uint64, osmomath.Dec) {
	...
	cmd := []string{"furyad", "tx", "concentratedliquidity", "create-position", fmt.Sprint(poolId), lowerTick, upperTick, tokens, fmt.Sprintf("%d", token0MinAmt), fmt.Sprintf("%d", token1MinAmt), fmt.Sprintf("--from=%s", from), "--gas=500000", "--fees=1250ufury", "-o json"}
...
func (n *NodeConfig) StoreWasmCode(wasmFile, from string) int {
	...
	cmd := []string{"furyad", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from), "--gas=auto", "--gas-prices=0.1ufury", "--gas-adjustment=1.3"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) WithdrawPosition(from, liquidityOut string, positionId uint64) {
	...
	cmd := []string{"furyad", "tx", "concentratedliquidity", "withdraw-position", fmt.Sprint(positionId), liquidityOut, fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000ufury"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) InstantiateWasmContract(codeId, initMsg, from string) {
	...
	cmd := []string{"furyad", "tx", "wasm", "instantiate", codeId, initMsg, fmt.Sprintf("--from=%s", from), "--no-admin", "--label=contract"}
...
func (n *NodeConfig) WasmExecute(contract, execMsg, from string) {
	...
	cmd := []string{"furyad", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	n.LogActionF(strings.Join(cmd, " "))
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryParams(subspace, key string) string {
	cmd := []string{"furyad", "query", "params", "subspace", subspace, key, "--output=json"}
...
func (n *NodeConfig) QueryGovModuleAccount() string {
	cmd := []string{"furyad", "query", "auth", "module-accounts", "--output=json"}
...
func (n *NodeConfig) SubmitParamChangeProposal(proposalJson, from string) int {
	...
	cmd := []string{"furyad", "tx", "gov", "submit-proposal", "param-change", fmt.Sprintf("/furya/param_change_proposal_%s.json", currentTime), fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SendIBCTransfer(dstChain *Config, from, recipient, memo string, token sdk.Coin) {
	...
	cmd := []string{"furyad", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, token.String(), fmt.Sprintf("--from=%s", from), "--memo", memo}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"furyad", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
// docker container exec <container id> furyad tx gamm swap-exact-amount-in <tokeinInCoin> <tokenOutMinAmountInt> --swap-route-pool-ids <swapRoutePoolIds> --swap-route-denoms <swapRouteDenoms> --chain-id=<id>--from=<address> --keyring-backend=test -b=block --yes --log_format=json
func (n *NodeConfig) SwapExactAmountIn(tokenInCoin, tokenOutMinAmountInt string, swapRoutePoolIds string, swapRouteDenoms string, from string) {
	...
	cmd := []string{"furyad", "tx", "gamm", "swap-exact-amount-in", tokenInCoin, tokenOutMinAmountInt, fmt.Sprintf("--swap-route-pool-ids=%s", swapRoutePoolIds), fmt.Sprintf("--swap-route-denoms=%s", swapRouteDenoms), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) JoinPoolExactAmountIn(tokenIn string, poolId uint64, shareOutMinAmount string, from string) {
	...
	cmd := []string{"furyad", "tx", "gamm", "join-swap-extern-amount-in", tokenIn, shareOutMinAmount, fmt.Sprintf("--pool-id=%d", poolId), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) ExitPool(from, minAmountsOut string, poolId uint64, shareAmountIn string) {
	...
	cmd := []string{"furyad", "tx", "gamm", "exit-pool", fmt.Sprintf("--min-amounts-out=%s", minAmountsOut), fmt.Sprintf("--share-amount-in=%s", shareAmountIn), fmt.Sprintf("--pool-id=%d", poolId), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitProposal(cmdArgs []string, isExpedited bool, propDescriptionForLogs string) int {
	...
	cmd := append([]string{"furyad", "tx", "gov", "submit-proposal"}, cmdArgs...)
...
func (n *NodeConfig) SubmitSuperfluidProposal(asset string) int {
	cmd := []string{"set-superfluid-assets-proposal", fmt.Sprintf("--superfluid-assets=%s", asset), "--title=\"superfluid asset prop\"", fmt.Sprintf("--description=\"%s superfluid asset\"", asset), "--from=val", "--gas=700000", "--fees=5000ufury"}
...
func (n *NodeConfig) SubmitCreateConcentratedPoolProposal(isExpedited bool) int {
	cmd := []string{"create-concentratedliquidity-pool-proposal", "--pool-records=stake,ufury,100,0.001", "--title=\"create concentrated pool\"", "--description=\"create concentrated pool", "--from=val"}
...
func (n *NodeConfig) DepositProposal(proposalNumber int, isExpedited bool) {
	...
	cmd := []string{"furyad", "tx", "gov", "deposit", fmt.Sprintf("%d", proposalNumber), deposit, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteYesProposal(from string, proposalNumber int) {
	...
	cmd := []string{"furyad", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "yes", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteNoProposal(from string, proposalNumber int) {
	...
	cmd := []string{"furyad", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "no", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) LockTokens(tokens string, duration string, from string) int {
	...
	cmd := []string{"furyad", "tx", "lockup", "lock-tokens", tokens, fmt.Sprintf("--duration=%s", duration), fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) AddToExistingLock(tokens osmomath.Int, denom, duration, from string, lockID int) {
	...
	path := fmt.Sprintf("/furya/lockup/v1beta1/locked_by_id/%d", lockID)
	...
	n.LogActionF("locking %s for %s", tokens, duration)
	cmd := []string{"furyad", "tx", "lockup", "lock-tokens", fmt.Sprintf("%s%s", tokens, denom), fmt.Sprintf("--duration=%s", duration), fmt.Sprintf("--from=%s", from)}
	_, _, err = n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SuperfluidDelegate(lockNumber int, valAddress string, from string) {
	...
	cmd := []string{"furyad", "tx", "superfluid", "delegate", lockStr, valAddress, fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSend(amount string, sendAddress string, receiveAddress string) {
	...
	cmd := []string{"furyad", "tx", "bank", "send", sendAddress, receiveAddress, amount, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) FundCommunityPool(sendAddress string, funds string) {
	...
	cmd := []string{"furyad", "tx", "distribution", "fund-community-pool", funds, fmt.Sprintf("--from=%s", sendAddress), "--gas=600000", "--fees=1500ufury"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateWallet(walletName string, chain *Config) string {
	...
	cmd := []string{"furyad", "keys", "add", walletName, "--keyring-backend=test"}
...
func (n *NodeConfig) AddExistingWallet(walletName, mnemonic string) {
	...
	cmd := []string{"sh", "-c", fmt.Sprintf("echo '%s' | furyad keys add %s --keyring-backend=test --recover", mnemonic, walletName)}
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"furyad", "keys", "show", walletName, "--keyring-backend=test"}
...
func (n *NodeConfig) QueryPropStatusTimed(proposalNumber int, desiredStatus string, totalTime chan time.Duration) {
	...
	require.Eventually(
		...
		1*time.Minute,
		10*time.Millisecond,
		"Furya node failed to retrieve prop tally",
	)
