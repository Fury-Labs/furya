...
func (suite *HooksTestSuite) TestWasmHooksAsyncAcks() {
	...
	allAcks := furyaApp.IBCKeeper.ChannelKeeper.GetAllPacketAcks(suite.chainA.GetContext())
	...
	_, err = suite.forceContractToEmitAckForPacket(furyaApp, suite.chainA.GetContext(), contractAddr, alreadyAckedPacket, true)
	...
	furyaApp.IBCHooksKeeper.SetParams(suite.chainA.GetContext(), params)
	...
	for _, tc := range testCases {
		...
		_, err = suite.forceContractToEmitAckForPacket(furyaApp, suite.chainA.GetContext(), contractAddr2, packet, tc.success)
		...
		_, err = suite.forceContractToEmitAckForPacket(furyaApp, ctx, contractAddr, packet, tc.success)
		totalExpectedAcks++
		suite.Require().NoError(err)
