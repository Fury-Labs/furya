...
type HooksTestSuite struct {
	...
	chainA *furyaibctesting.TestChain
	chainB *furyaibctesting.TestChain
	chainC *furyaibctesting.TestChain
...
func (suite *HooksTestSuite) SetupTest() {
	...
	suite.Setup()
	ibctesting.DefaultTestingAppInit = furyaibctesting.SetupTestingApp
	suite.coordinator = ibctesting.NewCoordinator(suite.T(), 3)
	suite.chainA = &furyaibctesting.TestChain{
		TestChain: suite.coordinator.GetChain(ibctesting.GetChainID(1)),
	}
	suite.chainB = &furyaibctesting.TestChain{
		TestChain: suite.coordinator.GetChain(ibctesting.GetChainID(2)),
	}
	suite.chainC = &furyaibctesting.TestChain{
...
func NewTransferPath(chainA, chainB *furyaibctesting.TestChain) *ibctesting.Path {
...
func (suite *HooksTestSuite) GetChain(name Chain) *furyaibctesting.TestChain {
	switch name {
	case ChainA:
		return suite.chainA
	case ChainB:
...
func (suite *HooksTestSuite) TestOnRecvPacketHooks() {
	...
	testCases := []struct {
		...
		{"override", func(status *testutils.Status) {
			suite.chainB.GetFuryaApp().TransferStack.
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			ack := suite.chainB.GetFuryaApp().TransferStack.
			...
			if _, ok := suite.chainB.GetFuryaApp().TransferStack.
...
func (suite *HooksTestSuite) receivePacketWithSequence(receiver, memo string, prevSequence uint64) []byte {
	...
	err := suite.chainB.GetFuryaApp().HooksICS4Wrapper.SendPacket(
...
func (suite *HooksTestSuite) TestFundsAreTransferredToTheContract() {
	...
	balance := suite.chainA.GetFuryaApp().BankKeeper.GetBalance(suite.chainA.GetContext(), addr, localDenom)
...
func (suite *HooksTestSuite) FullSend(msg sdk.Msg, direction Direction) (*sdk.Result, *sdk.Result, string, error) {
	var sender *furyaibctesting.TestChain
	switch direction {
	case AtoB:
		sender = suite.chainA
	case BtoA:
...
func (suite *HooksTestSuite) SetupPools(chainName Chain, multipliers []osmomath.Dec) []gammtypes.CFMMPoolI {
	...
	bondDenom := chain.GetFuryaApp().StakingKeeper.BondDenom(chain.GetContext())
	...
	for index, multiplier := range multipliers {
		...
		poolId, err := chain.GetFuryaApp().PoolManagerKeeper.CreatePool(chain.GetContext(), msg)
		suite.Require().NoError(err)

		pool, err := chain.GetFuryaApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
		suite.Require().NoError(err)
...
func (suite *HooksTestSuite) SetupCrosschainSwaps(chainName Chain, setupForwarding bool) (sdk.AccAddress, sdk.AccAddress) {
	...
	bankKeeper := chain.GetFuryaApp().BankKeeper
...
func (suite *HooksTestSuite) fundAccount(chain *furyaibctesting.TestChain, owner sdk.AccAddress) {
...
func (suite *HooksTestSuite) SetupCrosschainRegistry(chainName Chain) (sdk.AccAddress, string, string, string) {
	...
	// Fund the account with some ufury and some stake.
	for _, ch := range []*furyaibctesting.TestChain{suite.chainA, suite.chainB, suite.chainC} {
...
func (suite *HooksTestSuite) setChainChannelLinks(registryAddr sdk.AccAddress, chainName Chain) {
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			{"operation": "set","source_chain": "chainB","destination_chain": "furya","channel_id": "channel-0"},
			{"operation": "set","source_chain": "furya","destination_chain": "chainB","channel_id": "channel-0"},
			...
			{"operation": "set","source_chain": "furya","destination_chain": "chainC","channel_id": "channel-1"},
			{"operation": "set","source_chain": "chainC","destination_chain": "furya","channel_id": "channel-1"},
			{"operation": "set","source_chain": "furya","destination_chain": "chainB-cw20","channel_id": "channel-2"},
			{"operation": "set","source_chain": "chainB-cw20","destination_chain": "furya","channel_id": "channel-2"}
		  ]
		}
	  }
	  `
	...
	msg := `{
		"modify_chain_channel_links": {
		  "operations": [
			...
			{"operation": "change","source_chain": "chainB","destination_chain": "furya","new_source_chain": "chainD"},
			{"operation": "change","source_chain": "furya","destination_chain": "chainB","new_destination_chain": "chainD"}
		  ]
		}
	  }
	  `
...
func (suite *HooksTestSuite) TestCrosschainRegistry() {
	...
	contractAlias := "furya_registry_contract"
	...
	channelQuery := `{"get_channel_from_chain_pair": {"source_chain": "furya", "destination_chain": "chainB"}}`
	...
	_, err := suite.chainA.GetFuryaApp().WasmKeeper.QuerySmart(suite.chainA.GetContext(), registryAddr, []byte(channelQuery))
	suite.Require().Error(err)
	suite.Require().Contains(err.Error(), "RegistryValue not found")

	// Unwrap token0CB and check that the path has changed
	channelQuery = `{"get_channel_from_chain_pair": {"source_chain": "furya", "destination_chain": "chainD"}}`
	channelQueryResponse = suite.chainA.QueryContractJson(&suite.Suite, registryAddr, []byte(channelQuery))
	suite.Require().Equal("channel-0", channelQueryResponse.Str)
}
...
func (suite *HooksTestSuite) TestUnwrapToken() {
	...
	testCases := []struct {
		...
		{ChainA, "furya", token0CBA, token0CA, []Direction{AtoB, BtoC, CtoA}},
	...
	for _, tc := range testCases {
		...
		receiverApp := receiverChain.GetFuryaApp()
		initialSenderBalance := furyaApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
		...
		finalSenderBalance := furyaApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, tc.sentToken)
...
func (suite *HooksTestSuite) TestCrosschainSwaps() {
	...
	balanceSender := furyaApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")

	ctx := suite.chainA.GetContext()

	msg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
	...
	balanceSender2 := furyaApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCTest() {
	...
	furyaAppB := suite.chainB.GetFuryaApp()
	balanceToken0 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	receiver := initializer
	balanceToken1 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		receiver,
	)
	// Generate full memo
	...
	balanceToken0After := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadAck() {
	...
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr,
	...
	furyaAppA := suite.chainA.GetFuryaApp()
	balanceContract := furyaAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	swapMsg2 := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo": %s }}`,
		receiver,
		recoverAddr2,
		`{"wasm": "bad wasm specifier"}`,
	)
	...
	balanceContract2 := furyaAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	recoverMsg := `{"recover": {}}`
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(furyaAppA.WasmKeeper)
	_, err = contractKeeper.Execute(suite.chainA.GetContext(), crosschainAddr, recoverAddr2, []byte(recoverMsg), sdk.NewCoins())
	suite.Require().NoError(err)

	balanceRecovery := furyaAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), recoverAddr2, "token1")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadSwap() {
	...
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"min_output_amount":"50000"},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)
	// Generate full memo
	msg := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
...
func (suite *HooksTestSuite) TestBadCrosschainSwapsNextMemoMessages() {
	...
	innerMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s","on_failed_delivery": {"local_recovery_addr": "%s"},"next_memo":%%s}}`,
...
func (suite *HooksTestSuite) CreateIBCPoolOnChain(chainName Chain, denom1, denom2 string, amount1 osmomath.Int) uint64 {
	...
	_, err = chain.GetFuryaApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
	suite.Require().NoError(err)
	return poolId
}
...
func (suite *HooksTestSuite) TestCrosschainForwardWithMemo() {
	...
	balanceToken0IBCBefore := suite.chainA.GetFuryaApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
	...
	// Generate swap instructions for the contract
	//
	// Note: Both chains think of themselves as "furya" and the other as "chainB". That is, the registry
	...
	// be relevant here
	nextMemo := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": {"furya_swap":{"output_denom":"token0","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing"}}}}`,
		crosschainAddrB,
		receiver,
	)
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"ibc:channel-0/%s", "on_failed_delivery": "do_nothing", "next_memo": %s}}`,
		crosschainAddrB,
		nextMemo,
	)
	fmt.Println(swapMsg)
	...
	balanceToken0IBCAfter := suite.chainA.GetFuryaApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCMultiHop() {
	...
	furyaAppB := suite.chainB.GetFuryaApp()
	balanceToken0 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	balanceToken1 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	...
	balanceToken0After := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token0ACB)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), accountB, token1AB)
	suite.Require().Greater(balanceToken1After.Amount.Int64(), int64(0))
}
...
func (suite *HooksTestSuite) TestMultiHopXCS() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			sentTokenBalance := senderChain.GetFuryaApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			fmt.Println("sentTokenBalance", sentTokenBalance)
			suite.Require().True(sentTokenBalance.Amount.GTE(sendAmount))
			// get receiver balance
			receivedTokenBalance := receiverChain.GetFuryaApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)
			...
			swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s/%s", "on_failed_delivery": "do_nothing", "next_memo":{}%s}}`,
			...
			sentTokenAfter := senderChain.GetFuryaApp().BankKeeper.GetBalance(senderChain.GetContext(), tc.sender.address, initialToken)
			suite.Require().Equal(sendAmount.Int64(), sentTokenBalance.Amount.Sub(sentTokenAfter.Amount).Int64())

			fmt.Println(receiverChain.GetFuryaApp().BankKeeper.GetAllBalances(receiverChain.GetContext(), tc.receiver.address))

			receivedTokenAfter := receiverChain.GetFuryaApp().BankKeeper.GetBalance(receiverChain.GetContext(), tc.receiver.address, tc.receivedToken)
...
func (suite *HooksTestSuite) TestSwapErrorAfterPreSwapUnwind() {
	...
	sentTokenBalance := senderChain.GetFuryaApp().BankKeeper.GetBalance(senderChain.GetContext(), sender.address, initialToken)
	suite.Require().True(sentTokenBalance.Amount.GTE(sendAmount))

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"0"}},"receiver":"%s/%s", "on_failed_delivery": {"local_recovery_addr": "%s"}, "next_memo":{}}}`,
...
func (suite *HooksTestSuite) ExecuteOutpostSwap(initializer, receiverAddr sdk.AccAddress, receiver string) {
	...
	outpostAddr := suite.chainB.InstantiateContract(&suite.Suite,
		fmt.Sprintf(`{"crosschain_swaps_contract": "%s", "furya_channel": "channel-0"}`, crosschainAddr), 1)
	...
	furyaAppB := suite.chainB.GetFuryaApp()
	balanceToken0 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	balanceToken1 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)

	// Call the outpost
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(furyaAppB.WasmKeeper)
	...
	balanceToken1After := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)
