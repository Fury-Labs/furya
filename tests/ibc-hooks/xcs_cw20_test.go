...
func (suite *HooksTestSuite) setupCW20PoolAndRoutes(chain *furyaibctesting.TestChain, swaprouterAddr sdk.AccAddress, cw20IbcDenom string, amount osmomath.Int) {
	furyaAppA := chain.GetFuryaApp()
...
func (suite *HooksTestSuite) getCW20Balance(chain *furyaibctesting.TestChain, cw20Addr, addr sdk.AccAddress) osmomath.Int {
...
func (suite *HooksTestSuite) TestCW20ICS20() {
	...
	balanceStakeReceiver := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), stakeAB)
	suite.Require().Equal(int64(0), balanceStakeReceiver.Amount.Int64())

	// Transfer the tokens with a memo for XCS
	swapMsg := fmt.Sprintf(`{"furya_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
	...
	swapMsg = fmt.Sprintf(`{"furya_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB-cw20/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		cw20IbcDenom,
		receiver2,
	)
	xcsMsg = fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
	...
	balanceStakeReceiver2 := furyaAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), stakeAB)
