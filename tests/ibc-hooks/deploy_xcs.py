...
class Command(object):
    def __init__(self, command="furyad", node=None, keyring_backend=None, chain_id=None):
        self.command = command
        self.node = node or "http://localhost:26657"
        self.keyring_backend = keyring_backend or "test"
        self.chain_id = chain_id or "furya-1"
...
match ENV:
    ...
    case "edgenet":
        BASE_API = "https://api-furya.imperator.co"
        furyad = Command(node="https://rpc.edgenet.furya.zone:443", keyring_backend="test", chain_id="edgenet")
        CHAIN_REGISTRY_PATH = "~/devel/chain-registry"
    case "mainnet":
        SWAPROUTER_CODE_ID = 10
        CROSSCHAIN_SWAPS_CODE_ID = 31
        BASE_API = "https://api-furya.imperator.co"
        furyad = Command(node="https://rpc.furya.zone:443", keyring_backend="test", chain_id="furya-1")
        CHAIN_REGISTRY_PATH = "~/devel/chain-registry"
    case _:
...
async def get_gov_addr():
    module_accounts, err = await furyad.query("auth module-accounts", print_cmd=False)
...
async def get_channels_for_denoms(pools):
    ...
    async def fetch_trace(denom):
        trace, err = await furyad.query("ibc-transfer denom-trace", denom, print_cmd=False)
        if err:
            print(err)
            return None, denom
        return trace, denom
...
def get_denom_aliases():
    ...
    for chain_1_name, chain_2_name, channel in generate_channels():
        if "furya" not in chain_1_name.lower() and "furya" not in chain_2_name.lower():
            continue
        furya_chain, other_chain = ("chain_1", chain_2_name) if 'furya' in chain_1_name.lower() else ("chain_2", chain_1_name)
        furya_channel_id = channel[furya_chain]['channel_id']
        furya_port_id = channel[furya_chain]['port_id']

        channel_info = f"{furya_port_id}/{furya_channel_id}"
...
async def setup_registry(moniker, owner, gov, pools, dry_run=False):
    registry_id = get_code_id(
        await furyad(f"tx wasm store {REGISTRY_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))
    msg = '{"owner": "%s"}' % owner
    registry_addr = get_address(await furyad(
    ...
    result, err = await furyad(f"tx wasm execute {registry_addr}", msg,
...
async def setup_swaprouter(moniker, owner, gov, pools, dry_run=False):
    if SWAPROUTER_CODE_ID:
        ...
        swaprouter_id = get_code_id(
            await furyad(f"tx wasm store {SWAPROUTER_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))

    msg = '{"owner": "%s"}' % owner
    swaprouter_addr = get_address(await furyad(
    ...
    for message in swap_router_messages:
        result, err = await furyad(f"tx wasm execute {swaprouter_addr}", json.dumps(message),
                                     f"--from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run)
    ...
    # result, err = await furyad(f"tx wasm execute {swaprouter_addr}", '{"transfer_ownership": {"new_owner": "%s"}}' % gov,
    #                              f"--from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run)
    # if not isinstance(result, dict) or result['code'] != 0:
    #     print(result)
...
async def setup_xcs(moniker, governor, swaprouter_addr, registry_addr, gov, dry_run=False):
    if CROSSCHAIN_SWAPS_CODE_ID:
        ...
        xcs_id = get_code_id(
            await furyad(f"tx wasm store {CROSSCHAIN_SWAPS_PATH} --from {moniker}", GAS_ADJUSTMENT, dry_run=dry_run))

    msg = '{"governor": "%s", "swap_contract": "%s", "registry_contract": "%s"}' % (
        governor, swaprouter_addr, registry_addr)
    xcs_addr = get_address(await furyad(
...
async def deploy(dry_run=False, moniker="deployer", owner=None):
    (deployer, _), gov, pools = await asyncio.gather(
        furyad(f"keys show {moniker} -a", node=False, chain_id=False, block=False, as_json=False, print_cmd=False),
        get_gov_addr(),
        get_pools()
    )
