...
var (
	...
	externalGaugeCoins  = sdk.NewCoins(sdk.NewCoin("ufury", osmomath.NewInt(1000_000_000)))
...
func main() {
	...
	log.Printf("\n\n\nWARNING 1: your localfurya and client home are assummed to be %s. Run 'furyad get-env' and confirm it matches the path you see printed here\n\n\n", clientHome)
...
func createRandomPosition(igniteClient cosmosclient.Client, poolId uint64) (string, int64, int64, sdk.Coins, error) {
	...
	// 1 to 9. These are localfurya keyring test accounts with names such as:
	// lo-test1
	// lo-test2
	// ...
...
func getClientHomePath() string {
	...
	return currentUser.HomeDir + localfuryaFromHomePath
}
