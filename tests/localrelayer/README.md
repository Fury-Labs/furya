...
Localrelayer is a local testing environment composed of two localFurya instances connected by a relayer.
...
|------------------|------------|--------------------------|
| `localfurya-a` | `RPC`      | <http://localhost:26657> |
| `localfurya-a` | `REST/LCD` | <http://localhost:1317>  |
| `localfurya-a` | `gRPC`     | <http://localhost:9090>  |
| `localfurya-a` | `faucet`   | <http://localhost:8080>  |
| `localfurya-b` | `RPC`      | <http://localhost:36657> |
| `localfurya-b` | `REST/LCD` | <http://localhost:31317> |
| `localfurya-b` | `gRPC`     | <http://localhost:39090> |
| `localfurya-b` | `faucet`   | <http://localhost:38080> |
...
|------------------|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `localfurya-a` | `validator-a` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localfurya-a` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localfurya-a` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
| `localfurya-b` | `validator-b` | *family album bird seek tilt color pill danger message abuse manual tent almost ridge boost blast high comic core quantum spoon coconut oyster remove*            |
| `localfurya-b` | `faucet`      | *increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great*     |
| `localfurya-b` | `relayer`     | *black frequent sponsor nice claim rally hunt suit parent size stumble expire forest avocado mistake agree trend witness lounge shiver image smoke stool chicken* |
...
```bash
 ⠿ Network localrelayer_localfurya        Created
```

2. run the following containers:

```bash
 ⠿ Container localrelayer-localfurya-b-1  Created
 ⠿ Container localrelayer-localfurya-a-1  Created
...
d90ec29c7a6f   local:furya                  "/furya/setup.sh"      About a minute ago   Up 3 seconds   26656/tcp, 0.0.0.0:31317->1317/tcp, 0.0.0.0:39090->9090/tcp, 0.0.0.0:36657->26657/tcp   localrelayer-localfurya-b-1
e36cead49a07   local:furya                  "/furya/setup.sh"      About a minute ago   Up 3 seconds   0.0.0.0:1317->1317/tcp, 0.0.0.0:9090->9090/tcp, 0.0.0.0:26657->26657/tcp, 26656/tcp     localrelayer-localfurya-a-1
```
...
Check `localfurya-a` status:

```bash
curl -s http://localhost:26657/status
```

Check `localfurya-b` status:
...
```bash
❯ furyad keys add my-account --keyring-backend test
...
# Use the following endpoint for localfurya-b:
# FAUCET_ENDPOINT=http://localhost:38080

curl --header "Content-Type: application/json" \
  --request POST \
  --data '{"denom":"ufury","address":"osmo1e5zmvznxr0zuulsstna0rd3959sw858e5ctw2j"}' \
...
hermes tx ft-transfer --timeout-seconds 1000 \
    --dst-chain localfurya-a \
    --src-chain localfurya-b \
    --src-port transfer \
